...
namespace Event {
    ...
    static constexpr int32_t kMinDelay = 5;
    static constexpr int32_t kMaxDelay = 0x68D7A3;                                              // 6870947ms / 6870.947 seconds
    ...
    // 0x689D0.
    // ‣ if didn’t call system_timer_reinit has NOT been called, the timer value allowed range
    // from 5 to 0x68D7A3.
    ...
    constexpr milliseconds seconds(T seconds) {
        return milliseconds((milliseconds::rep)(seconds * 1000));
    }

    template<typename T>
    constexpr milliseconds minutes(T minutes) {
        return milliseconds((milliseconds::rep)(minutes * 60000));
    }
