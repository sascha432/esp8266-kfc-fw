/**
 * Author: sascha_lammers@gmx.de
 */

#include <Arduino_compat.h>
#include "SyslogParameter.h"
#include "Syslog.h"
#include "SyslogTCP.h"

#if DEBUG_SYSLOG
#include <debug_helper_enable.h>
#else
#include <debug_helper_disable.h>
#endif

SyslogTCP::SyslogTCP(SyslogParameter& parameter, const String &host, uint16_t port, bool useTLS) : Syslog(parameter) {
    _debug_printf_P(PSTR("SyslogTCP::SyslogTCP %s:%d TLS %d\n"), host.c_str(), port, useTLS);
    _host = host;
    _port = port;
    _useTLS = useTLS;
    _queue.callback = nullptr;
    _queue.isSending = false;
    _client.onConnect(_onConnect, this);
    _client.onDisconnect(_onDisconnect, this);
    _client.onAck(_onAck, this);
    _client.onError(_onError, this);
    _client.onData(_onData, this);
    _client.onPacket(_onPacket, this);
    _client.onTimeout(_onTimeout, this);
    _client.onPoll(_onPoll, this);
}

/**
 *
void SyslogTCP::__onConnect(AsyncClient *client) {
    __onPoll(client);
}void SyslogTCP::__onDisconnect(AsyncClient *client);
void SyslogTCP::__onError(AsyncClient *client, int8_t error);
void SyslogTCP::__onData(AsyncClient *client, void *data, size_t len);
void SyslogTCP::__onPacket(AsyncClient *client, struct pbuf *pb);
void SyslogTCP::__onTimeout(AsyncClient *client, uint32_t time);

 *
 */



void SyslogTCP::__onPoll(AsyncClient *client) {
    if (_queue.isSending) {
        if (client->canSend()) {
            auto space = client->space(); // there is at least one byte space
            auto send = _queue.message.length();
            if (send > space) {
                send = space;
            }
            auto written = client->write(_queue.message.getConstChar(), send);
            _debug_printf_P(PSTR("SyslogTCP::__onPoll(): space=%u,length=%u,send=%u,written=%u\n"), space, _queue.message.length(), send, written);
            if (written != send) {
                _queueWriteError();
            } else {
                _queue.written += written;
                _queue.message.removeAndShrink(0, written);
                if (_queue.message.length() == 0) {
                    _queueWritten(false); // data written, waiting for ack
                }
            }
        }
    } else {
        if (!_queue.isDisconnecting && millis() > _queue.idleTimeout) {
            _debug_printf_P(PSTR("SyslogTCP::__onPoll(): disconnecting idle connection\n"));
            _queue.isDisconnecting = 1;
            client->close(false);
        } else {
            _debug_printf_P(PSTR("SyslogTCP::__onPoll(): idle,is_disconnecting=%d\n"), _queue.isDisconnecting);
        }
#if DEBUG
        if (!_queue.idleTimeout) {
            _queueInvalidState();
        }
#endif
    }
}

void SyslogTCP::__onAck(AsyncClient *client, size_t len, uint32_t time) {
    _debug_printf_P(PSTR("SyslogTCP::__onAck(): len=%u,time=%u,queue:sending=%d,written=%u,sentAck=%u,left=%u\n"), len, time, _queue.isSending, _queue.written, _queue.sentAck, _queue.message.length());
    if (_queue.isSending) {
        _queue.sentAck += len;
        if (_queue.sentAck == _queue.written && _queue.message.length() == 0) {
            _queueWritten(true);
        }
    } else {
        _queueInvalidState();
    }
}



void SyslogTCP::_eventHandler(const char *name, AsyncClient *client, void *data) {
    _debug_printf_P(PSTR("SyslogTCP::_eventHandler(%s, %p, %p)\n"), name, client, data);
}

void SyslogTCP::_onConnect(void *arg, AsyncClient *client) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onConnect", client, nullptr);
    reinterpret_cast<SyslogTCP *>(arg)->__onPoll(client);
}

void SyslogTCP::_onDisconnect(void *arg, AsyncClient *client) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onDisconnect", client, nullptr);
    reinterpret_cast<SyslogTCP *>(arg)->_queueWriteError();
}

void SyslogTCP::_onAck(void *arg, AsyncClient *client, size_t len, uint32_t time) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onAck", client, nullptr);
    reinterpret_cast<SyslogTCP *>(arg)->__onAck(client, len, time);
}

void SyslogTCP::_onError(void *arg, AsyncClient *client, int8_t error) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onError", client, nullptr);
    reinterpret_cast<SyslogTCP *>(arg)->_queueWriteError();
}

void SyslogTCP::_onData(void *arg, AsyncClient *client, void *data, size_t len) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onData", client, nullptr);
}

void SyslogTCP::_onPacket(void *arg, AsyncClient *client, struct pbuf *pb) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onPacket", client, nullptr);
}

void SyslogTCP::_onTimeout(void *arg, AsyncClient *client, uint32_t time) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onTimeout", client, nullptr);
}

void SyslogTCP::_onPoll(void *arg, AsyncClient *client) {
    reinterpret_cast<SyslogTCP *>(arg)->_eventHandler("onPoll", client, nullptr);
    reinterpret_cast<SyslogTCP *>(arg)->__onPoll(client);
}

void SyslogTCP::transmit(const String &message, SyslogCallback callback) {
    if (_queue.isSending) {
        _debug_printf_P(PSTR("FATAL: Transmit called while sending\n"));
        panic();
    }
    _error = 0;
#if DEBUG
    _queue.idleTimeout = 0;
#endif
    _queue.isSending = true;
    // _queue.isDisconnecting = false;
    _queue.written = 0;
    _queue.sentAck = 0;
    _queue.message.clear();
    _queue.message.write(message.c_str(), message.length());
    _queue.message.write('\n');
    _queue.callback = callback;
    _connect();
}

bool SyslogTCP::isSending() {
	 return _queue.isSending;
}

void SyslogTCP::_connect() {
    _debug_printf_P(PSTR("SyslogTCP::_connect()\n"));
    if (!_isConnected()) {
        _error = 0;
        if (!_client.connect(_host.c_str(), _port)) {
            _debug_printf_P(PSTR("SyslogTCP::_connect(): _client.connect() failed\n"));
            _error++;
        }
    }
}

void SyslogTCP::_disconnect(bool now) {
    _debug_printf_P(PSTR("SyslogTCP::_disconnect(now=%d)\n"), now);
    _client.close(now);
}

bool SyslogTCP::_isConnected() {
    _debug_printf_P(PSTR("SyslogTCP::_isConnected(): connected=%d, connecting=%d, disconnecting=%d\n"), _client.connected(), _client.connecting(), _client.disconnecting());
    return _client.connected();
}

void SyslogTCP::_queueWritten(bool ack) {
    _debug_printf_P(PSTR("SyslogTCP::_queueWritten(%d): written=%u,left=0,sentAck=%u\n"), ack, _queue.written, _queue.sentAck);
    if (ack) {
        _queue.isSending = false;
        _queue.message.clear();
        _queue.callback(true);
        _queue.isDisconnecting = false;
        _queue.idleTimeout = millis() + (SYSLOG_TCP_MAX_IDLE * 1000UL);
        _debug_printf_P(PSTR("SyslogTCP::_queueWritten(): idleTimeout=%lu,timeout=%d\n"), _queue.idleTimeout, SYSLOG_TCP_MAX_IDLE);
    }
}

void SyslogTCP::_queueWriteError() {
    _debug_printf_P(PSTR("SyslogTCP::_queueWriteError(): written=%u,left=%u,sentAck=%u\n"), _queue.written, _queue.message.length(), _queue.sentAck);
    _queue.isSending = false;
    _queue.message.clear();
    _queue.callback(false);
    _error++;
    _disconnect(true);
}

void SyslogTCP::_queueInvalidState() {
    _debug_printf_P(PSTR("SyslogTCP::_queueInvalidState()\n"));
    _error++;
    _queueWriteError();
}
