...
void KFCFWConfiguration::restoreFactorySettings()
{
    ...
    #endif
    Logger_security(F("Factory settings restored...)"));
...
void KFCFWConfiguration::getStatus(Print &output)
{
        ...
        output.printf_P(PSTR("NVS flash storage max. size %uKB, %.1f%% in use" HTML_S(br)), config.getNVSFlashSize() / 1024, (stats.used_entries * 100) / float(stats.total_entries));
