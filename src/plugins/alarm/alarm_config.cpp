...
namespace KFCConfigurationClasses {
    ...
    namespace Plugins {
        ...
        namespace AlarmConfigNS {
            ...
            TimeType Alarm::getTime(const struct tm *_tm, SingleAlarmType &alarm)
            {
                if (!alarm.is_enabled) {
                    return 0;
                }
            ...
            void Alarm::updateTimestamps(const struct tm *tm, AlarmConfigType &cfg)
            {
                for(uint8_t i = 0; i < MAX_ALARMS; i++) {
            ...
            void Alarm::updateTimestamp(const struct tm *_tm, SingleAlarmType &alarm)
            {
            ...
            void Alarm::getWeekDays(Print &output, uint8_t weekdays, char none)
            {
            ...
            String Alarm::getWeekDaysString(uint8_t weekdays, char none)
            {
                PrintString str;
            ...
            void Alarm::dump(Print &output, AlarmConfigType &cfg)
            {
