...
void Sensor_AmbientLight::publishState()
{
    ...
    if (isConnected()) {
        auto value = (_sensor.type == SensorType::BH1750FVI && _sensor.bh1750FVI.highRes) ? _sensor.bh1750FVI.illuminance : getAutoBrightness();
        publish(_getTopic(), true, String(value, 0));
    }
}
