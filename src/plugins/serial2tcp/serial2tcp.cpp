...
void Serial2TcpPlugin::createConfigureForm(FormCallbackType type, const String &formName, Form &form, AsyncWebServerRequest *request)
{
    ...
    form.add<uint8_t>(FSPGM(mode), _H_W_STRUCT_VALUE(cfg, mode_byte))->setFormUI((new FormUI(FormUI::TypeEnum_t::SELECT, FSPGM(Mode)))->addItems(modes));
    ...
    form.add<uint32_t>(F("baud"), _H_W_STRUCT_VALUE(cfg, baudrate))->setFormUI((new FormUI(FormUI::TypeEnum_t::TEXT, F("Baud:")))->setPlaceholder(FSPGM(Default)));
    ...
    form.add(FSPGM(host), _H_STR_VALUE(MainConfig().plugins.serial2tcp.hostname))->setFormUI((new FormUI(FormUI::TypeEnum_t::TEXT, FSPGM(Hostname))));
    form.add<uint16_t>(FSPGM(port), _H_W_STRUCT_VALUE(cfg, port))->setFormUI((new FormUI(FormUI::TypeEnum_t::TEXT, FSPGM(Port))));
    ...
    form.add<bool>(FSPGM(auth), _H_W_STRUCT_VALUE(cfg, authentication))->setFormUI((new FormUI(FormUI::TypeEnum_t::SELECT, FSPGM(Authentication)))->setBoolItems());
    auto &authGroup = form.addDivGroup(F("auth_group"), F("{'i':'#auth','s':{'0':'$T.hide()','1':'$T.show()'}}"));
    form.add(F("s2t_user"), _H_STR_VALUE(MainConfig().plugins.serial2tcp.username))->setFormUI((new FormUI(FormUI::TypeEnum_t::TEXT, FSPGM(Username))));
    auto password = Serial2TcpPlugin::Serial2TCP::getPassword();
    form.add(F("s2t_pass"), _H_STR_VALUE(MainConfig().plugins.serial2tcp.password))->setFormUI((new FormUI(FormUI::TypeEnum_t::PASSWORD, FSPGM(Password)))->addAttribute(FSPGM(value), password));
    authGroup.end();
    authDivGroup.end();
